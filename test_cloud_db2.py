...
try:
    ...
    if result.data:
        print("Existing record columns:", list(result.data[0].keys()))
        print("Sample record:", result.data[0]['case_number'] if 'case_number' in result.data[0] else 'No case_number')
except Exception as e:
    print(f"Error reading: {e}")

# Create test record WITHOUT court_id
test_record = {
    "case_number": f"CLOUD-TEST-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
    # "court_id": "cloud_test",  # REMOVED
