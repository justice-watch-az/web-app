...
class TestMaricopaScraper:
    ...
    @patch('scrapers.maricopa_arraignment_scraper.webdriver.Chrome')
    def test_navigate_to_court(self, mock_chrome, mock_driver):
        """Test navigation to court page."""
        mock_chrome.return_value = mock_driver
        mock_element = Mock()
        ...
        scraper = MaricopaArraignmentScraper({'headless': True})
        scraper.navigate_to_court('Test Court')
        
        mock_element.click.assert_called_once()
    
    @patch('scrapers.maricopa_arraignment_scraper.webdriver.Chrome')
    def test_extract_case_data(self, mock_chrome, mock_driver):
        ...
        scraper = MaricopaArraignmentScraper({'headless': True})
        case_data = scraper.extract_case_data()
        
        assert case_data['case_number'] == 'TEST-001'
        assert case_data['case_title'] == 'State v. Test'