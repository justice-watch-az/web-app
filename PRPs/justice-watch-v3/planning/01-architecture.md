name: "Justice Watch v3.0 Serverless Architecture Planning"
description: "Complete architectural design for migrating from monolithic to serverless infrastructure (Netlify + Supabase + GitHub Actions)"
...
## Initial Concept
Migrate Justice Watch from monolithic Node.js/Docker architecture to serverless infrastructure using:
- **Frontend**: Netlify (static hosting + serverless functions)
- **Database**: Supabase (PostgreSQL + Auth + Real-time APIs)
- **Scraper**: GitHub Actions (Mac runner for anti-bot bypass)
- **Goal**: Achieve $0/month hosting costs while maintaining all functionality
