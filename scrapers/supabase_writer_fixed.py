#!/usr/bin/env python3
"""
Fixed Supabase writer for Maricopa County scraper.
Writes scraped arraignment data to production Supabase database.
Handles proxy parameter error in supabase v2.3.0.
"""
...
class SupabaseWriter:
    ...
    def __init__(self):
        ...
        # Clear proxy environment variables to avoid the proxy parameter issue
        proxy_vars = ['http_proxy', 'https_proxy', 'HTTP_PROXY', 'HTTPS_PROXY', 'no_proxy', 'NO_PROXY']
        original_proxies = {}
        for var in proxy_vars:
            if var in os.environ:
                original_proxies[var] = os.environ.pop(var)
                logger.info(f"Temporarily removed {var} from environment")
        
        try:
            # Import and create client after removing proxy vars
